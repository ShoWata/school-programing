<template>
  <div class="form py-8" id="form">
      ...
      <v-row>
        <v-col cols="6">
          <div
            ...
            class="mb-6"
          >
            <highlight :recievedCode="data.originCode"></highlight>
          </div>
        </v-col>
      </v-row>
    </div>
    ...
    <div
      id="rightAnswerEditor"
      class="mb-8 px-12"
    >
      <h3 class="mb-4">正解の編集</h3>
      <v-row>
        <template v-if="data.rightAnswers.length > 0">
          <v-col
            v-for="(answer, index) in data.rightAnswers"
            :key="index"
            cols="6"
          >
    ...
    <div
      ...
      class="mb-8 px-12"
    >
      <h3 class="mb-4">正解コード例の編集</h3>
      <v-row>
        <template v-if="data.rightCodes.length > 0">
          <v-col
            v-for="(code, index) in data.rightCodes"
            :key="index"
            cols="6"
          >
    ...
    read () {
      ...
      ref.collection('questions').doc(this.before.question.select)
        .get().then(doc => {
          ...
          this.data.originCode = doc.data().originCode
          this.data.rightAnswers = doc.data().rightAnswers
          this.data.rightCodes = doc.data().rightCodes
        })
    },
    ...
    postData () {
      ...
      ref.collection('questions').doc(this.after.question.select)
        .set({
          ...
          rightCodes: this.data.rightCodes
        })
        .then(() => {
          console.log('Docuemnts successfully written')
